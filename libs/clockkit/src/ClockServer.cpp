...
ClockServer::ClockServer(Poco::Net::IPAddress addr, int port, Clock& clock)
...
{
	Poco::Net::DatagramSocket socket(Poco::Net::SocketAddress(addr_, port_));
	...
	while (isRunning())
    {
		...
		int bytesReceived = socket.receiveFrom(buffer, length, peer);
        ...
        else
        {
            ...
            if (packet.getType() == ClockPacket::REQUEST)
            {
                ...
                packet.write(buffer);
                int bytesSent = socket.sendTo(buffer, length, peer);
                if (bytesSent != length)
                {
